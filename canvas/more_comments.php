...
if(mysqli_num_rows($cnv_q = mysqli_query($conx, "SELECT uid,username FROM accounts WHERE uid='$g_user'")) == '0') {
  header("location: /hub");
  exit();
}
...
$cv_qq = mysqli_query($conx, "SELECT * FROM canvas_comments WHERE uid_canvas='$cnv_uid' ORDER BY id DESC LIMIT $amount,5");
